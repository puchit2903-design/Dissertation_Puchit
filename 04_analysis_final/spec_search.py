...
# Engagement rate: no spec works, kept as a null finding
eng = d[d["engagement_rate"].notna() & (d["engagement_rate"] > 0)].copy()
eng["log_engage"] = np.log(eng["engagement_rate"])
# Engagement rate: same specification families as the pooled follower models,
# ordered by build-up, and repeated on the outfield-only sample
# Engagement rate: the same specification list applied to search interest
